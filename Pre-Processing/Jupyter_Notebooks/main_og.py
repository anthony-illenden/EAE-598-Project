...
def get_rel_vort(ds_pl, level):
    ...
    rel_vort = mpcalc.vorticity(u_sliced, v_sliced, dx, dy) * 1e5 # units: 1/s 
...
def get_tadv(ds_pl, level):
    ...
    tadv = mpcalc.advection(t_sliced, u=u_sliced, v=v_sliced, dx=dx, dy=dy) * 3600 * 3 # units: k/3 hr
...
def save_to_csv(ds_point, year, month, day, label):
